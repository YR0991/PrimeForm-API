...
2. **Wat "compleet" betekent (isProfileComplete):** fullName string ≥2 tekens; email string met '@'; birthDate YYYY-MM-DD; disclaimerAccepted === true; redFlags array length 0; goals array length 1–2; programmingType niet-leeg string; cycleData.lastPeriod YYYY-MM-DD; cycleData.avgDuration getal ≥21; cycleData.contraception niet-leeg string. Alle moeten true zijn.
...
|----------|------------|--------|----------|
| P2 | cycleData.lastPeriod vs lastPeriodDate: isProfileComplete checkt cycleData.lastPeriod; elders (cycleService) wordt lastPeriodDate gebruikt. | Mogelijk key-mismatch als frontend alleen lastPeriodDate stuurt. | Eén key (lastPeriodDate) of mapping in isProfileComplete. |
